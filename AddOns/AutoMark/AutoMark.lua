...
StaticPopupDialogs["AUTOMARK_NEW_CUSTOM_NPC"] = {
	...
	OnAccept = function (self, data, data2)
		local npcId = self.editBox:GetText()
...
StaticPopupDialogs["AUTOMARK_NEW_INSTANCE"] = {
	...
	OnShow = function (self, data)
		...
		if IsInInstance() then
			...
			if instanceID then
				self.editBox:SetText(instanceID)
			end
		end
		self.button1:Disable()
	end,
	OnAccept = function (self, data, data2)
		local instanceId = self.editBox:GetText()
...
GameTooltip:AddLine("Marker: " .. c .. tostring(MarkerName) .. FONT_COLOR_CODE_CLOSE,1,1,1)
...
for id, v in pairs(AutoMark_Instances) do
	...
	if not MyAddon.DefaultInstances[id] then
		...
		if v.name ~= MyAddon.Instances[id] then
			print("Custom Instance " .. id .. " is a default instance (" .. MyAddon.Instances[id] .. ").")
		else
			print("Custom Instance " .. id .. " is a default instance.")
		end
	end
