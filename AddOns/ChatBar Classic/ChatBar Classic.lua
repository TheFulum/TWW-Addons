...
Change Log:
v 3.7-WarWithin
- fixed WarWithin compatibality, 11.1.5 version
v 3.6-WarWithin
- fixed WarWithin release compatibility, 11.0.5 version
v 3.5-WWpre (Foxylittlething)
- fixed 11.0.0 and 11.0.2 versions compatibility
...
function ChatBar_ChannelShow(index)
	...
	if channelNum ~= 0 then
		if ChatBar_HideSpecialChannels then
			...
			if C_AddOns.IsAddOnLoaded("Sky") then
			...
			elseif C_AddOns.IsAddOnLoaded("CallToArms") and channelName == CTA_DEFAULT_RAID_CHANNEL then
				--Hide CallToArms channel
				return;
			elseif C_AddOns.IsAddOnLoaded("CT_RaidAssist") and channelName == CT_RA_Channel then
				--Hide CT_RaidAssist channel
				return;
			elseif C_AddOns.IsAddOnLoaded("GuildMap") and GuildMapConfig and channelName == GuildMapConfig.channel then
